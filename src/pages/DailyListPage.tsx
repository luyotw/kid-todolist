...
import { DAILY_LIST_COPY, type ListScope } from '../lib/listScope';
...
export default function DailyListPage({ scope }: { scope: ListScope }) {
  const copy = DAILY_LIST_COPY[scope];
  const now = useMemo(() => new Date(), []);
  const dateStr = formatDate(now);
  const { tasks } = useTasks();
  const { adhocToday, add: addAdhoc, remove: removeAdhoc } = useAdhoc(
    dateStr,
    scope,
  );
  const { completedIds, toggle } = useCompletions(dateStr, scope);
    ...
    <div className="today-page">
      ...
      {hasOverride && (
        <button
          ...
          onClick={() => restoreDefaultTodayOrder()}
        >
          恢復預設排序
        </button>
      )}
      ...
      {total === 0 ? (
        ...
        <SortableList
          items={sortableItems}
          onReorder={(keys) =>
            reorderToday(keys.map(parseTodayItemKey))
          }
            ...
            return (
              ...
              >
                <label>
                  ...
                  {row.source === 'adhoc' && (
                    <span className="today-row__badge">{copy.adhocBadge}</span>
                  )}
                </label>
                {row.source === 'adhoc' && (
                  <button
      ...
      <form className="adhoc-add" onSubmit={handleAddAdhoc}>
        <input
          ...
          aria-label={copy.adhocAriaLabel}
          placeholder={copy.adhocPlaceholder}
          value={adhocDraft}
      ...
      {allDone && (
        <div className="all-done" role="status" aria-live="polite">
          ...
          />
          <p className="all-done__heading">{copy.allDoneHeading}</p>
